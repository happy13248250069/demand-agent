...
interface Message {
  ...
  role: 'user' | 'agent';
  content: string;
  type: 'text' | 'table' | 'change-table' | 'rules-table' | 'validation-results' | 'sales-comparison-table' | 'external-info' | 'rule-explanation' | 'dp-table' | 'mnt-table' | 'simulation-ask' | 'version-select' | 'simulation-result' | 'import-confirm' | 'import-result' | 'validation-ask' | 'fcst-dimension-select';
  data?: any;
...
const AIPredictionTooltip = ({ simple = false }: { simple?: boolean }) => (
  <div className={`absolute hidden group-hover:block top-full left-1/2 -translate-x-1/2 mt-2 ${simple ? 'w-[380px]' : 'w-[420px]'} bg-white rounded-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.3)] z-[9999] text-left border border-gray-200 cursor-default p-5`}>
    <div className="flex flex-col gap-4">
      {!simple && (
        <>
          {/* 决策逻辑摘要 */}
          <div className="flex flex-col gap-2 bg-slate-50 border border-slate-100 rounded-lg p-3">
            <div className="text-[12px] font-black text-gray-900 mb-1">
              决策逻辑摘要
            </div>
            <ol className="list-decimal list-inside space-y-1 text-[11px] text-gray-700">
              <li>基准值取 供应上限 100。</li>
              <li>应用 <span className="font-bold text-red-500">外部事件溢价 (+14%)</span> 但抵消以 <span className="font-bold text-blue-500">历史虚高折扣 (-15%)</span>。</li>
              <li>综合计算：100 × (1 - 1%) ≈ 99。</li>
            </ol>
          </div>

          {/* 外部信息解读 */}
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between text-[13px] font-black text-gray-900 border-b border-gray-100 pb-1.5">
              <span>外部信息解读</span>
              <span className="text-red-500 font-bold">+14%</span>
            </div>
            <ul className="space-y-1 text-[11px] text-gray-600">
              <li><span className="font-semibold text-gray-800">事件：</span>检测到上游柔性折叠屏原材料涨价新闻 (来源：行业资讯)。</li>
              <li><span className="font-semibold text-gray-800">AI 标签判定：</span>上游供给-涨价（影响方向：正向，最大影响幅度预设为 +20%）。</li>
              <li><span className="font-semibold text-gray-800">相似度：</span>AI 分析该事件与该条数据的相似度为 0.7。</li>
              <li><span className="font-semibold text-gray-800">计算公式：</span>0.7 × (+20%) = +14%。</li>
              <li><span className="font-semibold text-gray-800">业务含义：</span>涨价预期可能导致客户提前囤货，存在真实需求放大的可能性。</li>
            </ul>
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <ul className="space-y-1 text-[11px] text-gray-600">
              <li><span className="font-semibold text-gray-800">ML 预测值：</span>115</li>
              <li><span className="font-semibold text-gray-800">与基准值对比：</span>115 &gt; 100，高出 15%</li>
              <li><span className="font-semibold text-gray-800">近6个月MAPE：</span>+30%</li>
              <li><span className="font-semibold text-gray-800">业务含义：</span>机器学习模型基于历史时序特征，提示该客户存在潜在需求增长趋势，但偏差较大，不予采纳。</li>
            </ul>
          </div>
        </>
      )}
      ...
      <div className="flex flex-col gap-2">
        ...
        <ul className="space-y-1 text-[11px] text-gray-600">
          <li><span className="font-semibold text-gray-800">匹配规则：</span>“原材料涨价”</li>
          <li><span className="font-semibold text-gray-800">填写原因汇总：</span>“涨价期间的申报量通常虚高，实际发货量比填报的少。”</li>
          <li><span className="font-semibold text-gray-800">平均调整幅度：</span>-20%</li>
          <li><span className="font-semibold text-gray-800">平均偏差：</span>+5%</li>
          <li><span className="font-semibold text-gray-800">建议系数：</span>平均调整幅度 - 平均偏差 = -15%</li>
          <li><span className="font-semibold text-gray-800">业务含义：</span>对于“原材料涨价”事件，销售人员平均下调20%，调整后的预测平均比实际出货高出5%，建议下调15%。</li>
        </ul>
      </div>
    </div>
    <div className="absolute bottom-full left-1/2 -translate-x-1/2 w-0 h-0 border-b-[8px] border-b-white border-x-[8px] border-x-transparent" />
    <div className="absolute bottom-full left-1/2 -translate-x-1/2 w-0 h-0 border-b-[9px] border-b-gray-200 border-x-[9px] border-x-transparent -z-10 -ml-[1px]" />
  </div>
);
  ...
  return (
    ...
    >
      ...
      {isAIPrediction ? (
        ...
        !showPopup && (aiSummary || isModified || (violatedRules && violatedRules.length > 0)) && (
